...
void wsolve_one_channel_grad(const vector<Point>& z, const vector<Point>& x, const Mat& ref_image, Mat& image, int width, const vector<double>& weights)
{
  /*
  -----x----
  |
  ...
  for(int k = 0; k < x.size(); ++k)
  {
    ...
    for(int irow = x[k].x - width/2; irow < x[k].x + width/2; ++irow)
    {
      for(int icol = x[k].y - width/2; icol < x[k].y + width/2; ++icol)
      {
        ...
        double gaussian_weight = exp(-(x_NH*x_NH+y_NH*y_NH)/(2*gamma*gamma));*/
        double gaussian_weight = 1.;
        double patch_weight = weights[k];

        sparseadd(A, icol*image_rows+irow, icol*image_rows+irow, gaussian_weight*patch_weight);
        b[icol*image_rows+irow] += patch_weight*gaussian_weight*ref_image.at<unsigned char>(z[k].x + (irow-x[k].x), z[k].y + (icol-x[k].y));
      }
    }
    ...
    for(int irow = x[k].x - width/2 +1; irow < x[k].x + width/2 -1; ++irow)
    {
      for(int icol = x[k].y - width/2 +1; icol < x[k].y + width/2 -1; ++icol)
      {
        ...
        // X direction gradient
        sparseadd(A, icol*image_rows+irow+1, icol*image_rows+irow+1, mu*gaussian_weight*patch_weight);
        sparseadd(A, icol*image_rows+irow+1, icol*image_rows+irow-1, -mu*gaussian_weight*patch_weight);
        b[icol*image_rows+irow+1] += mu*gaussian_weight*patch_weight*(ref_image.at<unsigned char>(z[k].x + (irow-x[k].x)+1, z[k].y + (icol-x[k].y)) - ref_image.at<unsigned char>(z[k].x + (irow-x[k].x)-1, z[k].y + (icol-x[k].y)));
        ...
        sparseadd(A, icol*image_rows+irow+1, icol*image_rows+irow+1, mu*gaussian_weight*patch_weight);
        sparseadd(A, icol*image_rows+irow-1, icol*image_rows+irow+1, -mu*gaussian_weight*patch_weight);
        b[icol*image_rows+irow+1] += mu*gaussian_weight*patch_weight*(ref_image.at<unsigned char>(z[k].x + (irow-x[k].x), z[k].y + (icol-x[k].y)+1) - ref_image.at<unsigned char>(z[k].x + (irow-x[k].x), z[k].y + (icol-x[k].y)-1));
...
void wsolve_grad(const std::vector<cv::Point>& z, const std::vector<cv::Point>& x, const cv::Mat& ref_image, cv::Mat& image, int width, const vector<double>& weights)
{
  Mat im_one_channel(image.size(), CV_8UC1);
  Mat ref_one_channel(ref_image.size(), CV_8UC1);
  ...
  for(int ichannel = 0; ichannel < 3; ++ichannel)
  {
    ...
    wsolve_one_channel_grad(z, x, ref_one_channel, im_one_channel, width, weights);
